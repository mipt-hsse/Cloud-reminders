...
document.addEventListener('DOMContentLoaded', function() {
  ...
  stage.on('mousedown.trdrag', (e) => {
    ...
    try {
      const name = (typeof target.name === 'function') ? target.name() : null;
      const anchorNames = new Set([
        'top-left', 'top-right', 'bottom-left', 'bottom-right', 'middle-left',
        'middle-right', 'top-center', 'bottom-center', 'rotater'
      ]);
    ...
    if (newTool === 'selection') {
      ...
      stage.on('mousedown.selection', e => {
        ...
        if (t.getParent() === tr || t === tr) {
          console.log(
              'Clicked on transformer or its anchor, skipping selection logic.');
          e.evt.stopPropagation();  // Останавливаем распространение DOM события
          return;
        }
      ...
      drawingLayer.listening(true);
      tr.nodes([
      ]);  // Снимаем выделение трансформатором, если перешли в режим размещения
      uiLayer.batchDraw();
      ...
      stage.on('click.placement', (e) => {
        if (e.target === stage) {  // Кликнули по пустому месту
          ...
          setTool(
              'selection');  // Возвращаемся в режим выделения после добавления
        }
      });
  ...
  stage.on('mousedown', function(e) {
    ...
    // обрабатывать событие Если tool не selection, то этот mousedown не должен
    ...
    if (tool === 'selection') {
      ...
      if (t.getParent() === tr || t === tr) {
        console.log(
            'Clicked on transformer or its anchor, stopping selection logic in general mousedown.');
        e.evt.stopPropagation();  // Останавливаем распространение DOM события
        return;
      }
      ...
      objectLayer.draw();
      drawingLayer.draw();
      uiLayer.batchDraw();  // Обновляем UI слой, чтобы трансформатор правильно
                            // отобразился/скрылся
    }

    // Handle double-click for stickers (остается здесь, так как это не зависит
    ...
    if (e.evt.detail === 2 && target && target.name() === 'sticker-group') {
      ...
      target.fire(
          'dblclick', {evt: e.evt});  // Fire custom dblclick event on the group
      e.evt.stopPropagation();  // Останавливаем распространение, чтобы избежать
                                // конфликтов
      return;
    }
  });
